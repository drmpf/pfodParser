...
static bool handleFileRead(String path) {
  if (!initializeFS()) {
    returnFail(FPSTR(FS_INIT_ERROR));
    return true;
  }
...
static void handleIndex() {
  ...
  // else
  if (handleFileRead("/localIndex.html")) {
    return;
  }
  sendFileNotFound("/localIndex.html");
}
...
void start_pfodWebServer(const char* version, const char* _pfodWebServerURL) {
  if (serverStarted) {
    return;
  }
  ...
  if (!pfodWebServerURL.length()) {  // empty url start LittleFS to server pages and .js
    if (!initializeFS()) {
      ...
      if (debugPtr) {
        ...
        if (debugPtr) {
          debugPtr->print(" Using pfodWebServer: ");
          debugPtr->print(pfodWebServerURL);
          debugPtr->println(" -- LittleFS not started here.");
        }
      }
    }
  }
